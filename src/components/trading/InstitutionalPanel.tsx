...
export function InstitutionalPanel({ verdict }: { verdict: InstitutionalVerdict | InstitutionalVerdictV2 | null | undefined }) {
  if (!verdict) {
    return (
  ...
  return (
    <div className="rounded-2xl border border-border bg-card/60 p-5 glass space-y-5">
        ...
        <div className="flex-1 space-y-3 min-w-0">
          ...
          {hasV2 && (
            <div className="flex flex-wrap gap-2">
              ...
              <Tag
                label="Composite"
                value={`${(v2.compositeScore?.value ?? 0) >= 0 ? "+" : ""}${v2.compositeScore?.value ?? 0} · ${v2.compositeScore?.regime ?? "N/A"}`}
                color={(v2.compositeScore?.value ?? 0) >= 25 ? "bull" : (v2.compositeScore?.value ?? 0) <= -25 ? "bear" : "gold"}
              />
              <Tag
                label="RSI Damping"
                value={(v2.components?.rsiPenalty ?? 0) !== 0
                  ? `${(v2.components?.rsiPenalty ?? 0) > 0 ? "+" : ""}${((v2.components?.rsiPenalty ?? 0) * 100).toFixed(0)}%`
                  : "محايد"}
                color={(v2.components?.rsiPenalty ?? 0) > 0 ? "bull" : (v2.components?.rsiPenalty ?? 0) < 0 ? "bear" : "muted"}
              />
              <Tag
                label="ATR Plan"
                value={(v2.targets?.side ?? "none") !== "none" ? `${(v2.targets?.side ?? "none") === "long" ? "Long" : "Short"} R:R ${v2.targets?.rr}` : "Standby"}
                color={(v2.targets?.side ?? "none") === "long" ? "bull" : (v2.targets?.side ?? "none") === "short" ? "bear" : "muted"}
              />
            </div>
          )}
      ...
      {hasV2 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          ...
          <div className="rounded-xl border border-border bg-card/40 p-3">
            ...
            <div className="mono text-[11px] mt-1 space-y-0.5">
              <div className="flex items-center gap-1">
                <SignIcon v={v2.components?.bookImbalance ?? 0} />
                قرب الدفتر: <span className={(v2.components?.bookImbalance ?? 0) > 0 ? "text-bull" : "text-bear"}>{((v2.components?.bookImbalance ?? 0) * 100).toFixed(0)}</span>
              </div>
              <div>انجراف ميكرو: <span className={(v2.components?.microDrift ?? 0) > 0 ? "text-bull" : "text-bear"}>{((v2.components?.microDrift ?? 0) * 100).toFixed(0)}</span></div>
              <div>قرب الجدران: <span className={(v2.components?.proximityPressure ?? 0) > 0 ? "text-bull" : "text-bear"}>{((v2.components?.proximityPressure ?? 0) * 100).toFixed(0)}</span></div>
              <div>تخفيف RSI: <span className={(v2.components?.rsiPenalty ?? 0) > 0 ? "text-bull" : (v2.components?.rsiPenalty ?? 0) < 0 ? "text-bear" : "text-muted-foreground"}>{((v2.components?.rsiPenalty ?? 0) * 100).toFixed(0)}</span></div>
              {typeof v2.scoreRaw === "number" && (
                <div className="text-muted-foreground">قبل التنعيم: {v2.scoreRaw > 0 ? "+" : ""}{v2.scoreRaw}</div>
              )}
              <div>صحة السبريد: <span className={(v2.components?.spreadHealth ?? 0) >= 0.6 ? "text-bull" : (v2.components?.spreadHealth ?? 0) >= 0.4 ? "text-gold" : "text-bear"}>{((v2.components?.spreadHealth ?? 0) * 100).toFixed(0)}%</span></div>
              <div>النظام: <span className="text-primary">{v2.compositeScore?.regime ?? "N/A"}</span></div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
        <Comp label="دفتر القرب" value={verdict?.components?.bookImbalance ?? 0} />
        <Comp label="ضغط الجدران" value={verdict?.components?.wallPressure ?? 0} />
        <Comp label="الزخم" value={verdict?.components?.momentum ?? 0} />
        <Comp label="اتجاه الحجم" value={verdict?.components?.volumeTrend ?? 0} />
        <Comp
          label="صحة الفارق"
          value={verdict?.components?.spreadHealth ?? 0}
          unsigned
        />
      </div>
